...
@section('header', 'Edit OPD: ' . $opd->name)

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <form action="{{ route('super-admin.opds.update', $opd) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <textarea name="description" 
                              id="description" 
                              rows="3"
                              class="w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500">{{ old('description', $opd->description) }}</textarea>
                    @error('description')
                        <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" 
                           name="head_name" 
                           id="head_name"
                           value="{{ old('head_name', $opd->head_name) }}"
                           class="w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500"
                           required>
                    @error('head_name')
                        <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="text" 
                           name="head_nip" 
                           id="head_nip"
                           value="{{ old('head_nip', $opd->head_nip) }}"
                           class="w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500">
                    @error('head_nip')
                        <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                    @enderror
